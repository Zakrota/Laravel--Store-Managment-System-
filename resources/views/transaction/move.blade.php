...
@section("title")
حركة اصناف لمخزن - نقل
@endsection()

@section("content")

<form method="post" class="ajaxForm" action="/transaction/storemove">
    {{csrf_field()}}
    <div class="row">
    <div class="col-sm-4">                
    ...
    <div class="col-sm-4">                
        <select name="to_store_id" id="to_store_id" class="form-control">
            <option value="">الى المخزن </option>
            @foreach($stores as $i)
                <option value="{{$i->id}}">{{$i->name}} 
                </option>
            @endforeach()
...
<script>    
    $(function(){
        $("#store_id").change(function(){
            $("#to_store_id").children().removeClass("hidden");
            if($(this).val()!=""){
                var v=$(this).val();
                $("#to_store_id").children().each(function(){
                    if($(this).val()==v){
        ...
        $("#to_store_id").change(function(){
            $("#store_id").children().removeClass("hidden");
            if($(this).val()!=""){
                var v=$(this).val();
                $("#store_id").children().each(function(){
                    if($(this).val()==v){
                        $(this).addClass("hidden");
                    }
                });
